...
const processMessage = async (
	...
	cacheControl: boolean | undefined, authstate : AuthenticationState
) => {
	...
	if (protocolMsg) {
		...
		switch (message.messageStubType) {
			...
			case WAMessageStubType.GROUP_CHANGE_ANNOUNCE: {
				...
				const isAnnounce = announceValue === 'true' || announceValue === 'on'
				emitGroupUpdate({ announce: isAnnounce })
				if (cacheControl) {
					caches.groupMetaCache.del(jid)
				}
				break
			...
			case WAMessageStubType.GROUP_CHANGE_RESTRICT: {
				...
				const isRestrict = restrictValue === 'true' || restrictValue === 'on'

				emitGroupUpdate({ restrict: isRestrict })
			...
			case WAMessageStubType.GROUP_MEMBER_ADD_MODE: {
				const mode = message.messageStubParameters?.[0]
				const isMemberAdd = mode === 'all_member_add'

				emitGroupUpdate({ memberAddMode: isMemberAdd })
			...
			case WAMessageStubType.GROUP_MEMBERSHIP_JOIN_APPROVAL_MODE: {
				...
				const isJoinApproval = approvalMode === 'on'

				emitGroupUpdate({ joinApprovalMode: isJoinApproval })
