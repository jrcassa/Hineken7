...
export const makeGroupsSocket = (config: SocketConfig) => {
	...
	const groupMetadata = async (jid: string, force?: boolean) => {
		if (cacheControl && !force) {
			const group = caches.groupMetaCache.get(jid) as GroupMetadata
			if (group) {
				return group
			}
		}
		const result = await groupQuery(jid, 'get', [{ tag: 'query', attrs: { request: 'interactive' } }])
		const groupReturn = extractGroupMetadata(result)
		if (cacheControl) {
			caches.groupMetaCache.set(jid, groupReturn)
		}
		return groupReturn
	}
