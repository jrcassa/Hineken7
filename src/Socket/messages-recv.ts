...
export const makeMessagesRecvSocket = (config: SocketConfig) => {
	...
	const msgRetryCache = config.msgRetryCounterCache || caches.msgRetryCache
	const callOfferCache = config.callOfferCache || caches.callOfferCache
	const placeholderResendCache = config.placeholderResendCache || caches.callOfferCache
	...
	const sendRetryRequest = async (node: BinaryNode, forceIncludeKeys = false) => {
		...
		let retryCount: number = Number(msgRetryCache.get<number>(key)) || 0
		...
		if (retryCount === 1) {
			if (authState.keys.remove) {
				const addrs = signalRepository.jidToSignalProtocolAddress(node.attrs.participant || node.attrs.from)
				await authState.keys.remove('session', [addrs])
			}

			let lids: string | undefined = undefined
			const retryLid = jidNormalizedUser(node.attrs.participant || node.attrs.from)
			if (!isLidUser(retryLid)) {
				lids = await getLid(retryLid)
			}
			await assertSessions([node.attrs.participant || node.attrs.from], true, lids)
		}
	...
	const processNotification = async (node: BinaryNode) => {
		...
		switch (nodeType) {
			...
			case 'devices':
				...
				const action = child.tag
				///fix old sessions on encript/decript message.
				if (action == 'remove') {
					for (const device of devices) {
						for (const key of ['jid', 'lid']) {
							const value = device.attrs[key]
							if (!value) continue
							if (authState.keys.remove) {
								const addrs = [value].map(jid => signalRepository.jidToSignalProtocolAddress(jid))
								await authState.keys.remove('session', addrs)
							}
							const normalized = jidNormalizedUser(value)
							caches.devicesCache.del(normalized)
							caches.devicesCache.del(`group-${normalized}`)
						}
					}
				}
				if (action == 'update') {
					const normalized = jidNormalizedUser(authState.creds.me?.lid)
					let lids: string | undefined = undefined
					if (!isLidUser(normalized)) {
						lids = await getLid(normalized)
					}
					caches.devicesCache.del(normalized)
					caches.devicesCache.del(`group-${normalized}`)
					if (lids) {
						caches.devicesCache.del(lids)
						caches.devicesCache.del(`group-${lids}`)
					}
				}
				if (action == 'add') {
					for (const device of devices) {
	...
	const willSendMessageAgain = (id: string, participant: string) => {
		...
		const retryCount: number = Number(msgRetryCache.get<number>(key)) || 0
		return retryCount < maxMsgRetryCount
	}

	const updateSendMessageAgainCount = (id: string, participant: string) => {
		const key = `${id}:${participant}`
		const newValue: number = Number(msgRetryCache.get<number>(key) || 0) + 1
		msgRetryCache.set(key, newValue)
	}

	const sendMessagesAgain = async (key: proto.IMessageKey, ids: string[], retryNode: BinaryNode) => {
		let msgs: (proto.IMessage | undefined)[] = []

		if (cacheControl) {
			msgs = await Promise.all(
				ids.map(async id => {
					const stored = (await caches.sendMessageCache.get(id)) as WAMessage
		...
		if (msgs.every(m => !m)) {
			msgs = await Promise.all(ids.map(async id => (await getMessage({ ...key, id })) ?? undefined))
		}
		...
		if (authState.keys.remove) {
			const addrs = signalRepository.jidToSignalProtocolAddress(participant)
			await authState.keys.remove('session', [addrs])
		} else {
	...
	const handleMessage = async (node: BinaryNode) => {
		...
		try {
			await Promise.all([
				processingMutex.mutex(async () => {
					let isplaceholder: WAMessageKey | undefined = undefined

					isplaceholder = caches.placeholderResendCache.get(`absentMessage-${msg.key.id}`) as WAMessageKey
					if (!isplaceholder) {
						const MessageAbsent = (await authState.keys.get('absentMessage', [msg.key.id!])) || {}
						const mAbsent = MessageAbsent[msg.key.id!]
						if (mAbsent) {
							isplaceholder = mAbsent as WAMessageKey
						}
					}
					if (isplaceholder) {
						if (authState.keys.remove) {
					...
					if (msg.messageStubType === proto.WebMessageInfo.StubType.CIPHERTEXT) {
						...
						*/
						await sendMessageAck(node)
						caches.placeholderResendCache.set(`absentMessage-${msg.key.id}`, msg.key)
						if (authState.keys.remove) {
							await authState.keys.set({
								absentMessage: { [msg.key.id!]: { messageKey: msg.key } }
							})
						}
						logger.info({ msg }, 'Mensagem perdida inseriada no store')
						...
						const isplaceholder = caches.placeholderResendCache.get(`absentMessage-${msg.key.id}`)
						if (isplaceholder) {
							caches.placeholderResendCache.del(`absentMessage-${msg.key.id}`)

							if (authState.keys.remove) {
								const MessageAbsent = (await authState.keys.get('absentMessage', [msg.key.id!])) || {}
								const mAbsent = MessageAbsent[msg.key.id!]
								if (mAbsent) {
									await authState.keys.remove('absentMessage', [msg.key.id!])
								}
								logger.info({ msg }, 'Mensagem recuperada e removida do store')
							}
						}
