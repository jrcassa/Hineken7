...
export const makeChatsSocket = (config: SocketConfig) => {
	...
	const placeholderResendCache = config.placeholderResendCache || (caches.placeholderResendCache as CacheStore)
	...
	const onWhatsApp = async (jids: string[] | string, force?: boolean): Promise<ProcessedContact[]> => {
		const results: ProcessedContact[] = []
		const missing: string[] = []

		const jidList = Array.isArray(jids) ? jids : [jids]

		const cleanJids = jidList.filter((jid): jid is string => typeof jid === 'string' && jid.length > 0)

		const normalize = (jid?: string) => {
			if (!jid || typeof jid !== 'string') return null
			const base = jid.replace('+', '').split('@')[0]?.split(':')[0]
			if (!base) return null
			return `${base}@s.whatsapp.net`
		}

		const normalizeNumber = (jid?: string) => {
			if (!jid || typeof jid !== 'string') return null

			return jid.replace('+', '').split('@')[0]?.split(':')[0] || null
		}
		...
		if (cacheControl && !force) {
			for (const jid of cleanJids) {
				...
				const cached = caches.onWhatsapp.get(key) as ProcessedContact
		...
		if (missing.length) {
			...
			const response = await sock.executeUSyncQuery(usyncQuery)

			if (response) {
				const processed: ProcessedContact[] = response.list.map(({ contact, id, lid }) => ({
					jid: id,
					exists: !!contact,
					lid: typeof lid === 'string' ? lid : undefined,
					ts: Date.now()
				}))

				for (const item of processed) {
					const key = normalize(item.jid)
					if (!key) continue

					results.push(item)

					if (item.lid) {
						caches.lidCache.set(key, item.lid)
					}

					if (cacheControl) {
						const ttl = item.exists ? 1800 : 300
						caches.onWhatsapp.set(key, item, ttl)
					}
				}
			}
		}
	...
	const updateBlockStatus = async (jid: string, action: 'block' | 'unblock'): Promise<undefined | boolean> => {
		let lid: string | undefined = undefined

		const check = await onWhatsApp([jid])
		if (!check[0].exists) {
			return undefined
		}
		...
		if (action === 'block') {
			const checkBusinessProfile = await getBusinessProfile(jid)
	...
	const profilePictureUrl = async (
		...
		if (cacheControl && !force) {
			const cached = caches.profilePicture.get(jid) as string
			if (cached) {
				return cached
			}
		}
		...
		const pictureUrl = child?.attrs?.url

		if (cacheControl && pictureUrl) {
			caches.profilePicture.set(jid, pictureUrl)
		}
	...
	const upsertMessage = ev.createBufferedFunction(async (msg: WAMessage, type: MessageUpsertType) => {
		if (msg.key.remoteJid && isLidUser(msg.key.remoteJid) && msg.key.sender_pn && !msg.key.fromMe) {
			msg.key.sender_lid = msg.key.remoteJid
			msg.key.remoteJid = msg.key.sender_pn
		}
		ev.emit('messages.upsert', { messages: [msg], type })

		if (msg.pushName) {
			let jid = msg.key.fromMe ? authState.creds.me!.id : msg.key.participant || msg.key.remoteJid
			const lid = msg.key.fromMe ? authState.creds.me!.lid : msg.key.participant || msg.key.remoteJid
			jid = jidNormalizedUser(jid!)
